...
class Import_customers extends App_import
{
    ...
    {
        $this->notImportableFields = hooks()->apply_filters('not_importable_clients_fields', ['userid', 'id', 'is_primary', 'password', 'company', 'vat', 'position', 'title', 'phonenumber', 'country', 'city', 'zip', 'state', 'address', 'website', 'billing_street', 'billing_city','billing_state','billing_zip', 'billing_country','shipping_street','shipping_city','shipping_city','billing_country', 'role', 'shipping_country','longitude','stripe_id','latitude','shipping_state','shipping_zip','datecreated', 'last_ip', 'last_login', 'last_password_change', 'active', 'new_pass_key', 'new_pass_key_requested', 'leadid', 'default_currency', 'profile_image', 'default_language', 'direction', 'show_primary_contact', 'invoice_emails', 'estimate_emails', 'project_emails', 'task_emails', 'contract_emails', 'credit_note_emails', 'ticket_emails', 'addedfrom', 'registration_confirmed', 'last_active_time', 'email_verified_at', 'email_verification_key', 'email_verification_sent_at']);
        ...
        foreach ($this->getRows() as $rowNumber => $row) {
            ...
            if (count($insert) > 0) {
                ...
                if (!$this->isSimulation()) {
                    ...
                    $insert['is_primary'] = 1;
                    $id                   = $this->ci->clients_model->add_($insert, true);
