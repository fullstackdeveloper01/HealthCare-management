...
class QuoteOfDay extends AdminController
{
    ...
    {
        if (!has_permission('quoteOfDay', '', 'view')) {
            access_denied('quoteOfDay');
        }
        if ($this->input->is_ajax_request()) {
            $this->app->get_table_data('quoteOfDay');
        }
       
        $subheader_text = setupTitle_text('aside_menu_active', 'reports', 'quoteOfDay');
        $data['sheading_text'] = $subheader_text;
        $data['sh_text'] = $subheader_text;
        
        $data['title']     = _l($subheader_text);
        $this->load->view('admin/quoteOfDay/quoteOfDays', $data);
    }
    ...
    public function add($id = '')
    {
        if ($this->input->post()) {
            ...
            if ($id == '') {
                if (!has_permission('quoteOfDay', '', 'create')) {
                    access_denied('quoteOfDay');
                }
                ...
                else
                {
                    ...
                    if ($id) {
                        set_alert('success', _l('added_successfully', _l('Quote of day')));
                        redirect(admin_url('quoteOfDay'));
                    }
                }
            } else {
                if (!has_permission('quoteOfDay', '', 'edit')) {
                    access_denied('quoteOfDay');
                }
                $success = $this->quoteofday_model->update_article($data, $id);
                
                if ($success) {
                    set_alert('success', _l('updated_successfully', _l('Quote of day')));
                }
                redirect(admin_url('quoteOfDay'));
            }
        }
        if ($id == '') {
            $title = _l('add_new', _l('Quote of day'));
        } else {
            $article         = $this->quoteofday_model->get($id);
           // echo '<pre>'; print_r($article); die;
            $data['article'] = $article;
            $title           = _l('edit', _l('Quote of day')) . ' ' . $article->name;
        }
        $subheader_text = setupTitle_text('aside_menu_active', 'reports', 'quoteOfDay');
        $data['sheading_text'] = $subheader_text;
        $data['sh_text'] = $subheader_text;
        ...
        $data['title']     = _l($subheader_text);
        $this->load->view('admin/quoteOfDay/quoteOfDay', $data);
    }
    ...
    public function sampledata() {
        ...
        header("Content-quoteOfDay: application/csv");
    ...
    public function delete_quoteOfDay($id)
    {
        if (!has_permission('quoteOfDay', '', 'delete')) {
            access_denied('quoteOfDay');
        }
        if (!$id) {
            redirect(admin_url('quoteOfDay'));
        }
        $response = $this->quoteofday_model->delete_article($id);
        if ($response == true) {
            set_alert('success', _l('deleted', _l('Quote of day')));
        } else {
            set_alert('warning', _l('problem_deleting', _l('Quote of day')));
        }
        redirect(admin_url('quoteOfDay'));
    }
}
