...
class ContactUs extends AdminController
{
    ...
    {
        if (!has_permission('contactUs', '', 'view')) {
            access_denied('contactUs');
        }
        if ($this->input->is_ajax_request()) {
            $this->app->get_table_data('contactUs');
        }
       
        $sheader_text = title_text('aside_menu_active', 'contactUs');
        $data['sheading_text'] = $sheader_text;
        $data['sh_text'] = $sheader_text;
        
        $data['title']     = _l($sheader_text);
        $this->load->view('admin/contactUs/contactUs', $data);
    }
