...
class Reports extends AdminController
{
    ...
    /* See service agreement reports*/
    public function service_agreement()
    { 
        $this->load->model('clientslo_model');
        $data['clo_list'] = $this->clientslo_model->getCloList();
        $sheader_text = title_text('aside_menu_active', 'service_agreement');
        $data['sheading_text'] = $sheader_text;
        $data['sh_text'] = $sheader_text;
        
        $data['title']     = _l($sheader_text);
        $this->load->view('admin/reports/service_agreement', $data);
    }

    /* See appointment reports*/
    ...
    { 
        ...
        $data['title']     = _l($sheader_text);
        $this->load->view('admin/reports/appointment', $data);
    }
     /* See request amendment reports*/
     public function request_amendment()
     { 
         $this->load->model('clientslo_model');
         $data['clo_list'] = $this->clientslo_model->getCloList();
         $sheader_text = title_text('aside_menu_active', 'request_amendment');
         $data['sheading_text'] = $sheader_text;
         $data['sh_text'] = $sheader_text;
         
         $data['title']     = _l($sheader_text);
         $this->load->view('admin/reports/requestamendment', $data);
     }

    /* See request amendment reports*/
    public function roster_reports()
    { 
        $this->load->model('clientslo_model');
        $data['clo_list'] = $this->clientslo_model->getCloList();
        ...
        $data['title']     = _l($sheader_text);
        $this->load->view('admin/reports/roster', $data);
    }
    ...
    { 
        ...
        $data['title']     = _l($sheader_text);
        $this->load->view('admin/reports/invoice', $data);
    }

    //Ndis-service-agreement-updated 
    public function ndis_service_agreement()
    {  
        //$this->session->set_userdata('auth', '{"id":"207","userid":"206","name":"Umar","email":"moumar212@mailinator.com","phone":"9039524321","dob":"2006-06-24","gender":"Male","role":"2","registration_no":"2736774","address":"Indore","token":"","profile_image":""}');
        $this->load->model('clientslo_model');
        $data['clo_list'] = $this->clientslo_model->getCloList();
        $sheader_text = title_text('aside_menu_active', 'ndis_service_agreement_updated');
        $data['sheading_text'] = $sheader_text;
        $data['sh_text'] = $sheader_text;
         
        $data['title']     = _l($sheader_text);
        $this->load->view('admin/reports/ndis_service_agreement_updated', $data);
    } 

    //Ndis-service-agreement-updated 
    public function loadNdisServiceAgreementClientsData(){
        
        $this->app->get_table_data('ndis_service_agreement_updated');
    }

    //Ndis-supplementary-service-agreement 
    public function ndis_supplementary_service_agreement()
    {  
        $this->load->model('clientslo_model');
        $data['clo_list'] = $this->clientslo_model->getCloList();
        $sheader_text = title_text('aside_menu_active', 'ndis_supplementary_service_agreement');
        $data['sheading_text'] = $sheader_text;
        $data['sh_text'] = $sheader_text;
        
        $data['title']     = _l($sheader_text);
        $this->load->view('admin/reports/ndis_supplementary_service_agreement', $data);
    } 

    //Ndis-supplementary-service-agreement 
    public function loadNdisSupplementaryServiceAgreementClientsData(){
        $this->app->get_table_data('ndis_supplementary_service_agreement');
    }

    //Private-clients-service-agreement
    public function ndis_private_clients_service_agreement()
    {
        $this->load->model('clientslo_model');
        $data['clo_list'] = $this->clientslo_model->getCloList();
        $sheader_text = title_text('aside_menu_active', 'ndis_private_clients_service_agreement');
        $data['sheading_text'] = $sheader_text;
        $data['sh_text'] = $sheader_text;
        
        $data['title']     = _l($sheader_text);
        $this->load->view('admin/reports/ndis_private_clients_service_agreement', $data);
    } 

    //Private-clients-service-agreement 
    public function loadNdisPrivateClientsServiceAgreementClientsData(){
        $this->app->get_table_data('ndis_private_clients_service_agreement');
    }
    //Ndis-service-agreement-updated 
    public function report_incident()
    {
        $this->load->model('clientslo_model');
        $data['clo_list'] = $this->clientslo_model->getCloList();
        $sheader_text = title_text('aside_menu_active', 'report_incident');
        $data['sheading_text'] = $sheader_text;
        $data['sh_text'] = $sheader_text;
        
        $data['title']     = _l($sheader_text);
        $this->load->view('admin/reports/report_incident', $data);
    } 

    //Ndis-service-agreement-updated 
    public function loadReportIncidentData(){
        $this->app->get_table_data('report_incident');
    }

     //my_aged_care_service_agrrement 
     public function my_aged_care_service_agreement()
     {
         $this->load->model('clientslo_model');
         $data['clo_list'] = $this->clientslo_model->getCloList();
         $sheader_text = title_text('aside_menu_active', 'my_aged_care_service_agrrement');
         $data['sheading_text'] = $sheader_text;
         $data['sh_text'] = $sheader_text;
         
         $data['title']     = _l($sheader_text);
         $this->load->view('admin/reports/ndis_my_aged_care_service_agreement_form', $data);
     } 
 
     //my_aged_care_service_agrrement 
     public function loadNdisMyAgedCareServiceAgreementData(){
         $this->app->get_table_data('ndis_my_aged_care_service_agreement_form');
     }

     
      //my_aged_care_service_agrrement 
      public function integrated_client_assessment_form()
      {
          $this->load->model('clientslo_model');
          $data['clo_list'] = $this->clientslo_model->getCloList();
          $sheader_text = title_text('aside_menu_active', 'integrated_client_assessment_form');
          $data['sheading_text'] = $sheader_text;
          $data['sh_text'] = $sheader_text;
          
          $data['title']     = _l($sheader_text);
          $this->load->view('admin/reports/ndis_integrated_client_assessment_form', $data);
      } 
  
      //my_aged_care_service_agrrement 
      public function loadNdiSintegratedClientAssessmentFormData(){
          $this->app->get_table_data('ndis_integrated_client_assessment_form');
      }
      public function editShowReportincident($id = '',$userid='')
      {
            ...
            $this->load->view('admin/reports/reportincidentstaffform', $data); 
      }
