...
class Clients extends AdminController
{
    ...
    public function client($id = '')
    {
        ...
        if ($this->input->post() && !$this->input->is_ajax_request()) {
            if ($id == '') {
                ...
                if($existuser != '')
                {
                    set_alert('warning', _l('Email is already exist'));
               ...
               // echo '<pre>'; print_r($data); die;
                ...
                $success = $this->clients_model->clientUpdate($this->input->post(), $id);
        ...
        if ($id == '') {
            ...
            if ($group == 'profile') {
                ...
                $data['propertyList'] = $this->db->order_by('id', 'desc')->get_where(db_prefix().'property', array('agent_id' => $client->userid))->result();
    ...
    */
    ...
    public function getAllCloClients(){
        $result= $this->clients_model->getAllCloClients();
        $clo_id= $_GET["cid"];
        $html = '';
        $html .= "<select class='form-control' data-live-search='true' id='cloclient-serviceagreementreport' onChange='getcloclientserviceagreementreport(".'this.value'.",".$clo_id.")' name='cloclient'><option value='0'>Filter By Client</option>";
        if(count($result)>0){
    ...
    {
        $result = $this->clients_model->loadServiceAgreementsCloClientData();
        $data = array();
        $no = $_POST['start'];
        ...
        $output = array(
            ...
            "recordsFiltered" => $this->clients_model->count_ServiceAgreementsCloClientFiltered(),
            "data" => $data,
        );
    ...
    */
    ...
    public function getAllCloClientsAppointment(){
        $result= $this->clients_model->getAllCloClients();
        $clo_id= $_GET["cid"];
        $html = '';
        $html .= "<select class='form-control' data-live-search='true' id='cloclient-appointmentreport' onChange='getcloclientappointmentreport(".'this.value'.",".$clo_id.")' name='cloclient'><option value='0'>Filter By Client</option>";
        if(count($result)>0){
            foreach($result as $row){
                $html .="<option value='".$row->userid."'>".$row->firstname." ".$row->lastname."</option>";
            }
    ...
    public function loadAppointmentsData(){
        $result = $this->clients_model->loadAppointmentCloClientData();
        $data = array();
        $no = $_POST['start'];
        if(count($result)>0){
            $i=0;
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => count($result),
            "recordsFiltered" =>$this->clients_model->count_AppointmentCloClientFiltered(),
            "data" => $data,
        );
    ...
    */
    ...
    public function getAllCloClientsRequestAmendment(){
        $result= $this->clients_model->getAllCloClients();
        $clo_id= $_GET["cid"];
        $html = '';
        $html .= "<select class='form-control' data-live-search='true' id='cloclient-requestamendmentreport' onChange='getcloclientrequestamendmentreport(".'this.value'.",".$clo_id.")' name='cloclient'><option value='0'>Filter By Client</option>";
        if(count($result)>0){
            foreach($result as $row){
                $html .="<option value='".$row->userid."'>".$row->firstname." ".$row->lastname."</option>";
            }
    ...
    public function loadRequestAmendmentClientData()
    {
        ...
        $result = $this->requestamendment_model->loadRequestAmendmentCloClientData();
        $data = array();
        $no = $_POST['start'];
        $i=0;
        foreach ($result as $e_res) 
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => count($result),
            "recordsFiltered" => $this->requestamendment_model->count_CloRequestAmendmentClientFiltered(),
            "data" => $data,
        );
        //output to json format
        echo json_encode($output);
    ...
    */
    ...
    public function loadServiceAgreementClientsData(){
        $this->app->get_table_data('service_agreement');
    }

    public function loadAppointmentsData(){
        $this->app->get_table_data('appointment_report');
    }

    public function loadRequestAmendmentClientData(){
        $this->app->get_table_data('request_amendment');
    }

    public function getAllCloClientsRoster(){
        $result= $this->clients_model->getAllCloClients();
        $clo_id= $_GET["cid"];
        $html = '';
        $html .= "<select class='form-control' data-live-search='true' id='cloclient-rosterreport' onChange='getcloclientrosterreport(".'this.value'.",".$clo_id.")' name='cloclient'><option value='0'>Filter By Client</option>";
        if(count($result)>0){
            foreach($result as $row){
                $html .="<option value='".$row->userid."'>".$row->firstname." ".$row->lastname."</option>";
            }
    ...
    public function loadRosterClientData(){
        $this->app->get_table_data('roster_report');
    }

    public function getAllCloClientsInvoice(){
        $result= $this->clients_model->getAllCloClients();
        $clo_id= $_GET["cid"];
        $html = '';
        $html .= "<select class='form-control' data-live-search='true' id='cloclient-invoicereport' onChange='getcloclientinvoicereport(".'this.value'.",".$clo_id.")' name='cloclient'><option value='0'>Filter By Client</option>";
        if(count($result)>0){
            foreach($result as $row){
                $html .="<option value='".$row->userid."'>".$row->firstname." ".$row->lastname."</option>";
            }
    ...
    public function loadInvoiceClientData(){
        $this->app->get_table_data('invoice_report');
    }
}
