...
class LeaveRequest extends AdminController
{
    ...
    {
        if (!has_permission('leaveRequest', '', 'view')) {
            access_denied('leaveRequest');
        }
        if ($this->input->is_ajax_request()) {
            $this->app->get_table_data('leaveRequest');
        }
       
        $subheader_text = setupTitle_text('aside_menu_active', 'reports', 'leave_request');
        $data['sheading_text'] = $subheader_text;
        $data['sh_text'] = $subheader_text;
        
        $data['title']     = _l($subheader_text);
        $this->load->view('admin/leaveRequest/leaveRequests', $data);
    }
    ...
    public function add($id = '')
    {
        if ($this->input->post()) {
            ...
            if ($id == '') {
                if (!has_permission('leaveRequest', '', 'create')) {
                    access_denied('leaveRequest');
                }
                ...
                if($existname)
                {
                    ...
                    redirect(admin_url('leaveRequest'));
                }
                else
                {
                    $data['created_date'] = date('Y-m-d H:i:s');
                    $id = $this->leaverequest_model->add_article($data);
                    if ($id) {
                ...
                if (!has_permission('leaveRequest', '', 'edit')) {
                    access_denied('leaveRequest');
                }
                $success = $this->leaverequest_model->update_article($data, $id);
        ...
        if ($id == '') {
            ...
            $article         = $this->leaverequest_model->get($id);
        ...
        $data['title']     = _l($subheader_text);
        $this->load->view('admin/leaveRequest/leaveRequest', $data);
    }
    ...
    public function sampledata() {
        ...
        header("Content-leaveRequest: application/csv");
    ...
    {
        if (!has_permission('leaveRequest', '', 'delete')) {
            access_denied('leaveRequest');
        }
        if (!$id) {
            redirect(admin_url('leaveRequest'));
        }
        $response = $this->leaverequest_model->delete_article($id);
        if ($response == true) {
