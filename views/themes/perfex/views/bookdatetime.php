    <?php
        if($setTime)
        {
            ...
            {
                ?>
                    <input type="radio" name="available_time" <?= ( in_array($r, $arr)?"disabled":""); ?> required id="one<?= $sn; ?>" value="<?= $r; ?>"><label class="label four col sellfield hover active text-danger" for="one<?= $sn; ?>"><span class="<?= ( in_array($r, $arr)?"text-danger":""); ?>"><?= $r; ?></span></label>
                <?php
                $sn++;
            }
        }
