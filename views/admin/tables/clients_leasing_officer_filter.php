...
$report_type =  $this->ci->db->escape_str($report_type);
$clo_status =  $this->ci->db->escape_str($clo_status);
$report_type_value = '';
$clo_status_value = 7;
if($report_type == 1)
{
	$report_type_value = 1;
	$report_type_value_s_date = date('Y-m-d 00:00:00');
	$report_type_value_e_date = date('Y-m-d 23:59:59');
}
elseif($report_type == 2)
{
	$report_type_value = 1;
	$report_type_value_s_date = date('Y-m-d 00:00:00', strtotime('-7 days'));
	$report_type_value_e_date = date('Y-m-d 23:59:59');
}
elseif($report_type == 3)
{
	$report_type_value = 1;
	$report_type_value_s_date = date('Y-m-d 00:00:00', strtotime('-1 month'));
	$report_type_value_e_date = date('Y-m-d 23:59:59');
}
elseif($report_type == 4)
{
	$report_type_value = 1;
	$report_type_value_s_date = date('Y-m-d 00:00:00', strtotime('-1 year'));
	$report_type_value_e_date = date('Y-m-d 23:59:59');
}
...
$aColumns = [
    ...
    db_prefix().'clients.role',
    'dob',
    'doj',
    db_prefix().'contacts.address as  address_new',
    'department_id',
...
//array_push($where, 'AND ('.db_prefix().'contacts.role = 2)');
array_push($where, 'AND '.db_prefix().'contacts.role = 3');

if ($this->ci->input->post('my_customers')) {
    array_push($where, 'AND '.db_prefix().'clients.userid IN (SELECT customer_id FROM '.db_prefix().'customer_admins WHERE staff_id=' . get_staff_user_id() . ')');
}

if($clo_status_value == 0)
{
	array_push($where, 'AND '.db_prefix().'clients.active=0');	
}
if($clo_status_value == 1)
{
...
{
	array_push($where, 'AND '.db_prefix().'clients.datecreated > "'.$report_type_value_s_date.'"');
	array_push($where, 'AND '.db_prefix().'clients.datecreated < "'.$report_type_value_e_date.'"');
}
...
foreach ($rResult as $aRow) {
    ...
    $company_  = $aRow['firstname'] . ' ' . $aRow['lastname'];
    ...
    $url = admin_url('clients_leasing_officer/client/' . $aRow['userid']);
    ...
    $company .= '<a href="' . admin_url('clients_leasing_officer/client/' . $aRow['userid'] . ($isPerson && $aRow['contact_id'] ? '?group=contacts' : '')) . '">' . _l('view') . '</a>';

    if ($aRow['registration_confirmed'] == 0 && is_admin()) {
        $company .= ' | <a href="' . admin_url('clients_leasing_officer/confirm_registration/' . $aRow['userid']) . '" class="text-success bold">' . _l('confirm_registration') . '</a>';
    }
    /*
    if (!$isPerson) {
        $company .= ' | <a href="' . admin_url('clients_leasing_officer/client/' . $aRow['userid'] . '?group=contacts') . '">' . _l('customer_contacts') . '</a>';
    }
    */
    if ($hasPermissionDelete) {
        $company .= ' | <a href="' . admin_url('clients_leasing_officer/delete/' . $aRow['userid']) . '" class="text-danger _delete">' . _l('delete') . '</a>';
    }
    ...
    // Primary contact
   // $row[] = ($aRow['contact_id'] ? '<a href="' . admin_url('clients_leasing_officer/client/' . $aRow['userid'] . '?contactid=' . $aRow['contact_id']) . '" target="_blank">' . $aRow['firstname'] . ' ' . $aRow['lastname'] . '</a>' : '');
    ...
    <input type="checkbox"' . ($aRow['registration_confirmed'] == 0 ? ' disabled' : '') . ' data-switch-url="' . admin_url() . 'clients_leasing_officer/change_client_status" name="onoffswitch" class="onoffswitch-checkbox" id="' . $aRow['userid'] . '" data-id="' . $aRow['userid'] . '" ' . ($aRow[db_prefix().'clients.active'] == 1 ? 'checked' : '') . '>
    ...
    $row[] = getDateTimeDMYOnly($aRow['datecreated']);

    
    $row[]   = icon_btn('clients_leasing_officer/add/' . $aRow['userid'], 'pencil-square-o');
