...
$aColumns = [
    ...
    db_prefix().'clients.added_by as added_byy',
    'company',
    'firstname',
    'email',
    db_prefix().'clients.phonenumber as phonenumber',
    db_prefix().'contacts.role',
...
if(isset($_GET['report_type']) && $_GET['report_type']!='')
{
    ...
    if($filter == 'day'){
        ...
        // array_push($where, 'AND '.db_prefix().'clients.datecreated="'.date('Y-m-d').'"');
        array_push($where, 'AND '.db_prefix().'clients.datecreated>="'.$from_date.'"');
        array_push($where, 'AND '.db_prefix().'clients.datecreated<="'.$to_date.'"');
    }
    ...
    if($filter == 'week'){
        ...
        // array_push($sql, 'AND '.db_prefix().'clients.datecreated>="'.date('Y-m-d').'"');
        // array_push($sql, db_prefix().'clients.datecreated<="'.date('Y-m-d',$date).'"');
    }
    ...
    if($filter == 'month'){
        ...
        // array_push($sql,'AND '. db_prefix().'clients.datecreated>="'.date('Y-m-d').'"');
        // array_push($sql, db_prefix().'clients.datecreated<="'.$date.'"');
    }
...
if(isset($_GET['client_status']) && $_GET['client_status']!=''){
    ...
    array_push($where, ' AND '.db_prefix().'clients.active='.$_GET['client_status']);
}
...
foreach ($rResult as $aRow) {
   ...
   // $row[] = ($aRow['contact_id'] ? '<a href="' . admin_url('clients/client/' . $aRow['userid'] . '?contactid=' . $aRow['contact_id']) . '" target="_blank">' . $aRow['firstname'] . ' ' . $aRow['lastname'] . '</a>' : '');
    ...
    <input onclick="changeStatus(this.value,'.$aRow['userid'].')"  type="checkbox"' . ($aRow['registration_confirmed'] == 0 ? ' disabled' : '') . ' data-switch-url="' . admin_url() . 'clients/change_client_status" name="onoffswitch" class="onoffswitch-checkbox" id="' . $aRow['userid'] . '" data-id="' . $aRow['userid'] . '" ' . ($aRow[db_prefix().'clients.active'] == 1 ? 'checked' : '') . ' value="'.$status.'">
    ...
    // $row[] = $groupsRow;
    $row[] =  $this->ci->db->get_where(db_prefix().'clients', array('userid' => $aRow['added_byy']))->row('company');
    ...
    $row[]   = $options .= icon_btn('clients/delete/' . $aRow['userid'], 'remove', 'btn-danger _delete');
