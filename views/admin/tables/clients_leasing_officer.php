...
foreach ($rResult as $aRow) {
   ...
   // $row[] = ($aRow['contact_id'] ? '<a href="' . admin_url('clients_leasing_officer/client/' . $aRow['userid'] . '?contactid=' . $aRow['contact_id']) . '" target="_blank">' . $aRow['firstname'] . ' ' . $aRow['lastname'] . '</a>' : '');
    ...
    $eDateStr .= '</ul>';
    $row[] = $eDateStr;
    $status= $aRow[db_prefix().'clients.active'];
    // Toggle active/inactive customer
    $toggleActive = '<div class="onoffswitch" data-toggle="tooltip" data-title="' . _l('customer_active_inactive_help') . '">
    <input onclick="changeStatus(this.value,'.$aRow['userid'].')" type="checkbox"' . ($aRow['registration_confirmed'] == 0 ? ' disabled' : '') . ' data-switch-url="' . admin_url() . 'clients_leasing_officer/change_client_status" name="onoffswitch" class="onoffswitch-checkbox" id="' . $aRow['userid'] . '" data-id="' . $aRow['userid'] . '" ' . ($aRow[db_prefix().'clients.active'] == 1 ? 'checked' : '') . ' value="'.$status.'">
    ...
    $options   = icon_btn('clients_leasing_officer/add/' . $aRow['userid'], 'pencil-square-o');

    $row[]   = $options .= icon_btn('clients_leasing_officer/delete_clients_leasing_officer/' . $aRow['userid'], 'remove', 'btn-danger _delete');
