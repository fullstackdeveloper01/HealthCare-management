...
$aColumns = [
   ...
   'ticketid',
    ...
    'CONCAT(' . db_prefix() . 'clients.company) as user_full_name',
    'status',
    'priority',
    'lastreply',
...
$additionalSelect = [
    ...
    db_prefix() . 'tickets.userid',
    db_prefix() . 'tickets.contactid',
    'assigned',
    db_prefix() . 'clients.company',
    ];

$join = [
    'LEFT JOIN ' . db_prefix() . 'contacts ON ' . db_prefix() . 'contacts.userid = ' . db_prefix() . 'tickets.contactid',
...
// die;
        ...
        if ($aColumns[$i] == '1') {
            ...
            if ($aRow['userid'] != 0) {
                $_data = '<a href="' . admin_url('clients/client/' . $aRow['userid'] . '?group=contacts') . '">' . $aRow['user_full_name'];
                if (!empty($aRow['company'])) {
                    $_data .= ' (' . getrolename($aRow['userid']) . ')';
                }
                $_data .= '</a>';
            } else {
            ...
            if ($aRow['contactid'] != 0) {
                $_data = $aRow['contact_full_name'];
                $_data .= '(' . getrolename($aRow['contactid']) . ')';
                 } else {
                $_data ='HR';
            }
