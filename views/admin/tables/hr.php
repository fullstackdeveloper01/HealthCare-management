...
if(isset($_GET['hr_status']) && $_GET['hr_status']!=''){
   
    array_push($where, ' AND '.db_prefix().'contacts.active='.$_GET['hr_status']);
}
$result  = data_tables_init($aColumns, $sIndexColumn, $sTable, [], $where, ['id','email','firstname','phonenumber','lastname','dob','doj']);
...
foreach ($rResult as $aRow) {
    ...
    <input onclick="changeStatus(this.value,'.$aRow['userid'].')" type="checkbox" data-switch-url="' . admin_url() . 'hr/change_emp_status" name="onoffswitch" class="onoffswitch-checkbox" id="' . $aRow['userid'] . '" data-id="' . $aRow['userid'] . '" ' . ($aRow['active'] == 1 ? 'checked' : '') . ' value="'.$status.'">
    ...
    $options = icon_btn('hr/add/' . $aRow['userid'], 'pencil-square-o');
    $row[]   = $options .= icon_btn('hr/delete_hr/' . $aRow['userid'], 'remove', 'btn-danger _delete');
