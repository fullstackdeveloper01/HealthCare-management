...
foreach($statuses as $status){
  ...
  <div class="col-md-2 col-xs-6 mbot15 border-right">
    <a href="#" data-cview="ticket_status_<?php echo $status['ticketstatusid']; ?>" onclick="resetValue();dt_custom_view('ticket_status_<?php echo $status['ticketstatusid']; ?>','.tickets-table','ticket_status_<?php echo $status['ticketstatusid']; ?>',true); return false;">
 ...
 <script>
   function resetValue() 
   {
    $('.tickets_filters input').each(function(){
        $(this).val('') 
    });
   }
 </script>
